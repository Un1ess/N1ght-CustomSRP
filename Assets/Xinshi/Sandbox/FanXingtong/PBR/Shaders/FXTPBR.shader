        ...
        Pass
        {
            ...
            half4 FXTLitPassFragment(Varyings input) : SV_Target
            {
               ...
               //eturn half4(1,1,0,1);
                ...
                half3 lightColorForSSS = mainLight.color *mainLight.distanceAttenuation;
                direct_lighting = BSDFDirect_SGSSS(brdfData,inputData,lightDirWS,lightColorForSSS,energyCompensationTerm,SSSColor);
                IndirectTerm += _AmbientSkinColor;
                    ...
                    for (uint lightIndex = 0u; lightIndex < pixelLightCount; ++lightIndex)
                    {
                        ...
                        float3 addLightDirWs = addlight.direction;
                        float3 addLightColor = addlight.color * addlight.distanceAttenuation * addlight.shadowAttenuation;
                        direct_lighting.diffuse += BSDFDirect_SGSSS(brdfData, inputData, addLightDirWs, addLightColor,energyCompensationTerm,SSSColor).diffuse;
                        direct_lighting.specular += BSDFDirect_SGSSS(brdfData, inputData, addLightDirWs, addLightColor,energyCompensationTerm,SSSColor).specular;
                ...
                return half4(indirect_lighting.diffuse,alpha);
            }
            ENDHLSL
