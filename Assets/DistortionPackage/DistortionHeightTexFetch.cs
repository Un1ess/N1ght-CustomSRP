...
/// </summary>
public class DistortionHeightTexFetch : MonoBehaviour
{
    private const int MaxHeightTextureSize = 2048;
    public LayerMask layerMask = -1;
    public float meterPerPix = 1;
    [Min(0)] public int extendCount = 1;
    public Texture2D bakedHeightMap;
    private Renderer soulVisualRenderer;
#if UNITY_EDITOR
    [ContextMenu("生成高度图")]
    public void BakeHeightTex()
    {
        ...
        var depthCopyShader = Shader.Find("distortion/DistortionHeightFetchCopyDepth");
        var savePath = path.Replace(extendName, "_HeightMap.png");
        bakedHeightMap = GetDepth(center, size, height, extendCount, layerMask, texSize, depthCopyShader,
            savePath);
        ...
        // Shader.SetGlobalTexture("_HeightMap",bakedHeightMap);
        // Shader.SetGlobalVector("_HeightMapCenterRange", new Vector4(center.x, center.y, center.z, size));
        // Shader.SetGlobalFloat("_HeightMapDepth", height);

        // SoulVisualDistortion soulVisualDistortion = UniversalRenderPipeline.asset.scriptableRenderer.GetFeature<SoulVisualDistortion>();
        var renderer = UniversalRenderPipeline.asset.GetRenderer(0);
        SoulVisualDistortion soulVisualDistortion = null;
        if (soulVisualDistortion != null)
        {
            soulVisualDistortion.heightMask = bakedHeightMap;
            soulVisualDistortion.heightMapCenterRange = new Vector4(center.x, center.y, center.z, size);
            soulVisualDistortion.heightMapDepth = height;
        }
        else
        {
            Debug.LogWarning("soulVisualDistortion Feature为空");
        }
       
    }
#endif

    public static Texture2D GetDepth(Vector3 pos, float orthographicSize, float maxDistance, int extendCount,
        LayerMask layerMask, int texSize, Shader depthCopyShader, string savePath)
    {
        ...
        {
            tempTex2.filterMode = tempTex1.filterMode = depthRT.filterMode = FilterMode.Point;
